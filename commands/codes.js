...
Commands.claimcode = function(Common, from, to, message) {
	...
	if (Common.utils.msg(message)) {
		...
		Common.db.users.findOne({name: name}, function(err, user) {
			...
			} else if (user.claimCodeTest == code[1] || user.oct2018 == code[1] || user.nov2018 == code[1] || user.nov1st2018sl == code[1] || user.dec2018 == code[1] || user.dec1st2018sl == code[1] || user.dec28th2018sl == code[1] || user.jan2019 == code[1] || user.jan21st2019sl == code[1] || user.feb2019 == code[1] || user.feb20th2019sl == code[1] || user.mar2019 == code[1] || user.mar19th2019sl == code[1] || user.apr2019 == code[1] || user.apr12th2019sl == code[1] || user.may2019 == code[1] || user.may12th2019sl == code[1]) {
...
//					var codes = 0;
//					users.forEach(function(may2019) {
//						codes++;
//					});
//					if (codes == 1) {
//						Common.bot.say(to, "5Sorry " + from + ", this code has been claimed the maximum number of times: " + code[1] + ".");
//					} else {
						Common.db.users.update({name: name}, {$set: {may2019: code[1]}}, {upsert: false}, function(err, updated) {
							if (err || !updated) {
								console.log('Error', err);
							} else {
								Common.bot.say(to, "3" + from + ", you have successfully claimed this code: " + code[1] + ". You have unlocked the May 2019 command: tba");
							}
						});
//					}
//				});
...
Commands.may2019 = function(Common, from, to, message) {
	name = Common.utils.toDb(from);
	Common.db.users.findOne({name: name}, function(err, user) {
		if (err || !user) {
			console.log(err);
			Common.bot.say(to, "5This command may be unlocked by claiming the promotion code for May 2019.");
		} else {
			if (user.may2019 == 'may2019') {
				Common.bot.say(to, "working!!");
			} else {
				Common.bot.say(to, "5This command may be unlocked by claiming the promotion code for May 2019.");
			}
