		...
		if (err || !channel) {
			...
			if (typeof time[1] != 'undefined') {
				...
				if (time[1] == '0' || time[1] == '1' || Common.utils.toLc(time[1]) == 'now') {
					...
					Common.bot.say(to, "2" + from + " plans to leave games sometime soon - use !gameWarn NUMBER_HERE to give your official game warning, use !d when you leave games, and reassign roles if necessary.");
				} else {
					var gwmins = gw * 13;
					if (!isNaN(gwmins)) {
