...
Commands.spotlight = function(Common, from, to, message) {
	var today = new Date();
	var slEndPrev = new Date("2018-10-11T00:00:00Z");
	var slStart = new Date("2018-11-01T00:00:00Z");
	var slEnd = new Date("2018-11-04T00:00:00Z");
	var slStart2 = new Date("2018-12-1T00:00:00Z");
	var slDays = '';
	var slHours = '';
	var slMinutes = '';
	var slSeconds = '';
	...
	if ((today>slStart && today<slEnd) || today<slStart) {
		sldate = "Thu Nov 1st-Sun Nov 4th 2018 00:00 GT";
		prevsldate = "Mon Oct 8th-Thu Oct 11th 2018 00:00 GT";
	} else if (today>slEnd) {
		...
		Common.db.channels.findOne({channel: to}, function(err, channel) {
			if (err || !channel) {
				...
				if (ops[to].indexOf(from) > -1 || halfops[to].indexOf(from) > -1)  {
					...
					var nick  = message.match(/\S+/g);
					if (typeof nick[1] != 'undefined') {
						Common.bot.say(to, "5This command may only be used by staff members to force set a role to a specified member. Use !ROLE_HERE to set a role randomly, or use !rolerequest ROLE_HERE NAME to request a role to be assigned to a member.");
					} else {
						from = 'random';
						Common.utils.pickUser(Common, to, role_type, from);
					}
