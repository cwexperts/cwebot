...
Common.bot.addListener('message', function(from, to, message) {
    ...
    if (Common.utils.isCmd(message)
      && typeof(Common.commands[command]) === 'function') {
        Common.commands[command](Common, from, to, message);
    } else if (Common.utils.isCmd(message)
      && typeof(Common.commands[command]) !== 'function') {
...
//		}, 10000);
		...
		if (Common.utils.toLc(nick) != 'abdel' && Common.utils.toLc(nick) != 'hanna' && Common.utils.toLc(nick) != 'alan_' && Common.utils.toLc(nick) != 'alan__' && Common.utils.toLc(nick) != 'base_tank' && Common.utils.toLc(nick) != 'fable' && Common.utils.toLc(nick) != 'ipso') {
